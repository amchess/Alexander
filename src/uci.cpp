...
UCI::UCI(int argc, char** argv) :
    ...
    options["Threads"] << Option(1, 1, 1024, [this](const Option&) {
        threads.set({bookMan, options, threads, tt});
    });
    ...
    options["UCI_LimitStrength"] << Option(
      false, [this](const Option&) { Eval::initHandicapMode(options); });
    options["Handicapped Depth"] << Option(
      false, [this](const Option&) { Eval::initHandicapMode(options); });
    options["UCI_Elo"] << Option(3190, 1320, 3190,
                                 [this](const Option&) { Eval::initHandicapMode(options); });
    options["Handicapped avatar player"]
      << Option(false, [this](const Option&) { Eval::initHandicapMode(options); });
    options["Avatar File"] << Option("", [](const Option& o) { Eval::loadAvatar(o); });
    //only for chessbase gui handicap mode begin
    options["LimitStrength_CB"] << Option(
      false, [this](const Option&) { Eval::initHandicapMode(options); });
...
void UCI::loop() {
    ...
    pos.set(StartFEN, false, &states->back(), threads.main_thread());
        ...
        // These commands must not be used during a search!
            ...
            sync_cout
              << "\nAlexander is a powerful chess engine for playing and analyzing."
                 ...
                 "\nFor any further information, visit https://github.com/official-alexander/Alexander#readme"
...
void UCI::go(Position& pos, std::istringstream& is, StateListPtr& states) {
    ...
    if (limits.perft)
    {
        perft(pos.fen(), limits.perft, options["UCI_Chess960"],
              threads.main_thread());  //from classical eval
        return;
    }
...
void UCI::trace_eval(Position& pos) {
    StateListPtr states(new std::deque<StateInfo>(1));
    Position     p;
    p.set(pos.fen(), options["UCI_Chess960"], &states->back(), threads.main_thread());
...
void UCI::search_clear() {
    ...
    // livebook begin
#ifdef USE_LIVEBOOK
    Alexander::Search::set_livebook_retry((int) options["Live Book Retry"]);
    Alexander::Search::set_livebook_depth((int) options["Live Book Depth"]);
#endif
    // livebook end
    ...
    Search::initWinProbability();             //from Alexander
}
...
void UCI::position(Position& pos, std::istringstream& is, StateListPtr& states) {
    ...
    pos.set(fen, options["UCI_Chess960"], &states->back(), threads.main_thread());
