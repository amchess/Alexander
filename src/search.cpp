...
inline int8_t getInitialShashinWinProbabilityRange(Position& pos, Stack* ss) {
    if (!highTal && !middleTal && !lowTal && !capablanca && !highPetrosian && !middlePetrosian
        && !lowPetrosian)
        return getShashinRange(static_value(pos, ss), pos);
...
inline void initShashinValues(Position& pos, Stack* ss, Worker* worker) {
    worker->shashinPly                 = std::max(pos.game_ply(), ss->ply);
    worker->shashinWinProbabilityRange = getInitialShashinWinProbabilityRange(pos, ss);
}
// end from Shashin
...
void Search::Worker::iterative_deepening() {
    ...
    initShashinValues(rootPos, ss, this);  //from Shashin
...
void putGameLineIntoLearningTable(Position& rootPos) {
    ...
    auto pos = Position();
    pos.set(rootPos.fen(), rootPos.is_chess960(), rootPos.state(), nullptr);
