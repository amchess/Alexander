...
struct std::hash<Alexander::FixedString<N>> {
    std::size_t operator()(const Alexander::FixedString<N>& fstr) const noexcept {
        return Alexander::hash_bytes(fstr.data(), fstr.size());
    }
};
