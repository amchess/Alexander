...
// ====================================================================
// FUNZIONI ORIGINALI GLOBALI (Fuori dal namespace anonimo!)
// ====================================================================
...
// ====================================================================
// FUNZIONI HELPER LOCALI (Inizio Namespace Anonimo)
// ====================================================================
namespace {

// Funzione Helper locale: Bitboard veloci
bool isPawnNearPromotion(const Position& pos) {
    return (pos.pieces(WHITE, PAWN) & (Rank5BB | Rank6BB | Rank7BB))
        || (pos.pieces(BLACK, PAWN) & (Rank2BB | Rank3BB | Rank4BB));
}

// Funzione Helper locale: Ottimizzata
bool isComplex(const Position& pos) {
    // 1. Pedoni vicini alla promozione (Velocissimo)
    if (isPawnNearPromotion(pos))
        return true;

    // 2. Pericolo Re (Indipendente da Shashin: controlliamo se il Re è sotto scacco ora)
    if (pos.checkers())
        return true;

    // 3. Materiale Alto (Velocissimo)
    bool highMaterial = (pos.non_pawn_material(WHITE) + pos.non_pawn_material(BLACK)) > 2400;

    // 4. Mosse Legali (LENTO - Viene eseguito solo se tutto il resto è falso)
    // Ottimizzazione Short-Circuit: MoveList viene generata solo qui.
    if (highMaterial)
    {
        return MoveList<LEGAL>(pos).size() >= 25;
    }
...
// ====================================================================
// FUNZIONI OTTIMIZZATE (Spostate nel namespace anonimo per evitare warning)
// ====================================================================

// Function to compute the adjusted complexity factor based on Elo and game phase
// OPTIMIZED: Accepts pre-calculated isPosComplex
double computeAdjustedComplexityFactor(int uciElo, const Position& pos, bool isPosComplex) {
    // Complexity factor based on position
    double complexityFactor = isPosComplex ? 1.0 : 0.5;
...
// OPTIMIZED: Accepts pre-calculated isPosComplex
bool should_apply_perturbation(int uciElo, const Position& pos, bool isPosComplex) {
    double complexityFactor = computeAdjustedComplexityFactor(uciElo, pos, isPosComplex);
...
// OPTIMIZED: Accepts pre-calculated complexPosition boolean
Value get_handicap_value(Value           baseEvaluation,
                         int             uciElo,
                         const Position& pos,
                         bool            complexPosition) {

    // Logic for error ranges
    struct HandicapErrorRange {
        int minErrorMagnitude;
        int maxErrorMagnitude;
    };

    static const HandicapErrorRange handicapErrorRanges[] = {
    ...
    // Safety swap if ranges are inverted
    if (minError > maxError)
        std::swap(minError, maxError);
...
}  // End Anonymous Namespace (Qui finiscono tutte le helper private)


// ====================================================================
// FUNZIONI PUBBLICHE (Queste DEVONO essere visibili)
// ====================================================================

// Main function (Orchestrator)
Value get_perturbated_value(const Position& pos, Value baseEvaluation) {
    // 1. Calculate Complexity ONCE here (Expensive part)
    bool isPosComplex = isComplex(pos);

    // 2. Pass the result down to avoid re-calculation
    if (should_apply_perturbation(handicapConfig.uciElo, pos, isPosComplex))
    {
        baseEvaluation =
          get_handicap_value(baseEvaluation, handicapConfig.uciElo, pos, isPosComplex);
    }
