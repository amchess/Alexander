...
__attribute__((hot)) bool ShashinManager::useMoveGenCrystalLogic() const {
    ...
    return Alexander::isInRange(state.dynamicBase.currentRange, ShashinPosition::MIDDLE_LOW_TAL,
...
__attribute__((hot)) bool ShashinManager::useStep17CrystalLogic() const {
    ...
    // Uso l'helper
    if (__builtin_expect(Alexander::isInRange(state.dynamicBase.currentRange,
...
bool ShashinManager::isTillCategory(ShashinPosition lowerBound, ShashinPosition upperBound) const {
    ...
    return Alexander::isInRange(state.dynamicBase.currentRange, lowerBound, upperBound);
}
